...
//Metodo md5, utilizar cuando sea necesario
	...
	public static String[][] Sucesores(Cubo c){
		...
		int dimension=c.getPosiciones()[0].length; //obtenemos la dimension de la matriz sucesores a partir del tamaño del cubo
		String sucesores [][]= new String [dimension*6][3]; //matriz de sucesores que almacena [movimiento,nuevoestado,coste]
		...
		for(int i=0; i<dimension;i++){   //rellenamos la matriz en trozos de 6 "i" veces( el numero de caras por la dimension del cubo)
			...
			sucesores[i*6][0]="L"+i;sucesores[i*6][1]=id;sucesores[i*6][2]=Integer.toString(costActi);
			id = Estado.obtenerID(Movimiento.movimiento(c, 'l', i));
			sucesores[(i*6)+1][0]="l"+i;sucesores[(i*6)+1][1]=id;sucesores[(i*6)+1][2]=Integer.toString(costActi);
			id = Estado.obtenerID(Movimiento.movimiento(c, 'D', i));
			sucesores[(i*6)+2][0]="D"+i;sucesores[(i*6)+2][1]=id;sucesores[(i*6)+2][2]=Integer.toString(costActi);
			id = Estado.obtenerID(Movimiento.movimiento(c, 'd', i));
			sucesores[(i*6)+3][0]="d"+i;sucesores[(i*6)+3][1]=id;sucesores[(i*6)+3][2]=Integer.toString(costActi);
			id = Estado.obtenerID(Movimiento.movimiento(c, 'B', i));
			sucesores[(i*6)+4][0]="B"+i;sucesores[(i*6)+4][1]=id;sucesores[(i*6)+4][2]=Integer.toString(costActi);
			id = Estado.obtenerID(Movimiento.movimiento(c, 'b', i));
			sucesores[(i*6)+5][0]="b"+i;sucesores[(i*6)+5][1]=id;sucesores[(i*6)+5][2]=Integer.toString(costActi);
		}
